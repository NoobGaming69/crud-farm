            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4>Common Usage
                            <a href="common_usage-create.php" class="btn btn-primary float-end">Add Common Usage</a>
                        </h4>
                    </div>
                    <div class="card-body">
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php
                                $query = pg_query($con, "select common_usage.*, crops.crop_name
                                FROM common_usage
                                JOIN crops ON common_usage.crop_id = crops.crop_id order by usage_id");

                                if (pg_num_rows($query) > 0) {
                                    while ($common_usage = pg_fetch_assoc($query)) {
                                        // Your code inside the loop
                                ?>
                                        <tr>
                                            <td><?= $common_usage['usage_id']; ?></td>
                                            <td><?= $common_usage['crop_name']; ?></td>
                                            <td><?= $common_usage['usage_name']; ?></td>
                                            <td><?= $common_usage['description']; ?></td>
                                            <td><?= $common_usage['image']; ?></td>
                                            <td><?= $common_usage['usage_example']; ?></td>
                                            <td>
                                                <a href="common_usage-view.php?usage_id=<?= $common_usage['usage_id']; ?>" class="btn btn-info btn-sm">View</a>
                                                <a href="common_usage-update.php?usage_id=<?= $common_usage['usage_id']; ?>" class="btn btn-success btn-sm">Update</a>
                                                <form action="code.php" method='POST' class="d-inline">
                                                    <button type="submit" name="delete_common_usage" value="<?= $common_usage['usage_id']; ?>" class="btn btn-danger btn-sm">Delete</a>
                                                </form>
                                            </td>
                                        </tr>
                                <?php
