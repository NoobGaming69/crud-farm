            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4>Governance
                            <a href="governance-create.php" class="btn btn-primary float-end">Add Governance</a>
                        </h4>
                    </div>
                    <div class="card-body">
                        ...
                        <table class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    <th>Governance ID</th>
                                    <th>Tribe Name</th>
                                    <th>Tribal Governance</th>
                                    <th>Intertribal Relations</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                $query = pg_query($con, "select governance.*, tribe.name
                                FROM governance
                                JOIN tribe ON governance.tribe_id = tribe.tribe_id order by governance_id");

                                if (pg_num_rows($query) > 0) {
                                    while ($governance = pg_fetch_assoc($query)) {
                                        // Your code inside the loop
                                ?>
                                        <tr>
                                            <td><?= $governance['governance_id']; ?></td>
                                            <td><?= $governance['name']; ?></td>
                                            <td><?= $governance['tribal_governance']; ?></td>
                                            <td><?= $governance['intertribal_relations']; ?></td>
                                            <td>
                                                <a href="governance-view.php?governance_id=<?= $governance['governance_id']; ?>" class="btn btn-info btn-sm">View</a>
                                                <a href="governance-update.php?governance_id=<?= $governance['governance_id']; ?>" class="btn btn-success btn-sm">Update</a>
                                                <form action="code.php" method='POST' class="d-inline">
                                                    <button type="submit" name="delete_governance" value="<?= $governance['governance_id']; ?>" class="btn btn-danger btn-sm">Delete</a>
                                                </form>
                                            </td>
                                        </tr>
                                <?php
